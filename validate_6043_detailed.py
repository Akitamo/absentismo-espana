"""
Validador detallado tabla 6043: Nacional + Secciones CNAE + Jornada
...
def read_and_prepare_ine_csv(file_path):
    """
    Lee y prepara el CSV del INE tabla 6043
    """
    print("=" * 80)
    print("LEYENDO DATOS DEL INE - TABLA 6043")
    print("=" * 80)
    ...
    # Mostrar algunas secciones CNAE encontradas
    print("\nAlgunas secciones CNAE encontradas:")
    # El nombre de columna es 'Secciones de la CNAE-09' según vemos arriba
    secciones = df_2025['Secciones de la CNAE-09'].unique()
...
def get_duckdb_data_detailed(db_path):
    """
    Extrae datos detallados de DuckDB para tabla 6043
    """
    print("\n" + "=" * 80)
    print("EXTRAYENDO DATOS DE DUCKDB - TABLA 6043")
    print("=" * 80)
    
    conn = duckdb.connect(str(db_path))
    
    # Query completa para obtener todos los datos de 2025T1 tabla 6043
    query = """
    SELECT 
        tipo_jornada,
        cnae_nivel,
    ...
    # Mostrar secciones CNAE cargadas
...
def create_detailed_comparison(df_ine, df_db):
    """
    Crea comparación detallada entre INE y DuckDB para tabla 6043
    """
    print("\n" + "=" * 80)
    ...
    # Mapeo de secciones CNAE (basado en exploración validada)
    seccion_map = {
    ...
    for idx, row_ine in df_ine.iterrows():
        ...
        seccion_ine = row_ine['Secciones de la CNAE-09']
        ...
        # Intentar mapear la sección CNAE
        cnae_nivel = None
        cnae_codigo = None
        ...
        if metrica_db and cnae_nivel:
            ...
            if len(df_match) > 0:
                ...
                no_encontrados.append((jornada_ine, seccion_ine, tiempo_ine))
                comparisons.append({
                    'Tipo Jornada': jornada_ine,
                    'Sección CNAE': seccion_ine[:50],
                    'Métrica': tiempo_ine,
    ...
    if no_encontrados:
        ...
        for i, (jornada, seccion, metrica) in enumerate(no_encontrados[:3]):
            print(f"  - {jornada} / {seccion[:40]} / {metrica}")
...
def create_validation_excel(df_ine, df_db, df_comparison, output_path):
    ...
    ws2['A1'] = "VALIDACIÓN TABLA 6043 - PERIODO 2025T1"
    ws2['A1'].font = Font(bold=True, size=14)
    ws2['A1'].alignment = Alignment(horizontal='center')
    
    ws2['A3'] = "Secciones CNAE con Tipo de Jornada"
...
def main():
    """
    Función principal de validación tabla 6043
    """
    print("\n" + "=" * 80)
    print("VALIDACIÓN DETALLADA TABLA 6043")
    print("Nacional + Secciones CNAE + Tipo de Jornada")
    print("Comparación INE CSV vs DuckDB - Periodo 2025T1")
    print("=" * 80 + "\n")
    
    # Configuración de rutas
    base_dir = Path(__file__).parent
    csv_file = base_dir / "data" / "INE" / "6043.csv"
    db_path = base_dir / "data" / "analysis.db"
    output_file = base_dir / "data" / "INE" / "validacion_6043_detallada.xlsx"
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("VALIDACIÓN COMPLETADA - TABLA 6043")
        print("=" * 80)
