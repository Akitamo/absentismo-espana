"""
Validador detallado tabla 6044: Nacional + Sectores B-S (sin jornada)
...
def read_and_prepare_ine_csv(file_path):
    """
    Lee y prepara el CSV del INE tabla 6044
    """
    print("=" * 80)
    print("LEYENDO DATOS DEL INE - TABLA 6044")
    print("=" * 80)
    ...
    # Buscar columna de sectores (puede tener diferentes nombres)
    sector_col = None
    for col in df_2025.columns:
        if 'Sectores' in col or 'CNAE' in col:
            sector_col = col
            break
    
    if sector_col:
        sectores = df_2025[sector_col].unique()
        for sector in sectores[:5]:
            print(f"  - {sector[:60]}...")
        if len(sectores) > 5:
            print(f"  ... y {len(sectores)-5} más")
    
    return df_2025, sector_col

def get_duckdb_data_detailed(db_path):
    """
    Extrae datos detallados de DuckDB para tabla 6044
    """
    print("\n" + "=" * 80)
    print("EXTRAYENDO DATOS DE DUCKDB - TABLA 6044")
    print("=" * 80)
    
    conn = duckdb.connect(str(db_path))
    
    # Query completa para obtener todos los datos de 2025T1 tabla 6044
    # NOTA: tabla 6044 NO tiene tipo_jornada (es NULL)
    query = """
    SELECT 
        tipo_jornada,
        cnae_nivel,
    ...
    # Verificar tipo_jornada (debe ser NULL para 6044)
    ...
    # Mostrar sectores B-S
    sectores_db = df[df['cnae_nivel'] == 'SECTOR_BS']['cnae_codigo'].unique()
    print(f"\nSectores B-S en BD: {sorted([s for s in sectores_db if s])}")
    
    return df

def create_detailed_comparison(df_ine, df_db, sector_col):
    """
    Crea comparación detallada entre INE y DuckDB para tabla 6044
    """
    print("\n" + "=" * 80)
    ...
    # Mapeo de sectores (basado en tabla 6042 que ya funciona)
    sector_map = {
    ...
    for idx, row_ine in df_ine.iterrows():
        ...
        if metrica_db and cnae_nivel:
            ...
            # Para tabla 6044, tipo_jornada debe ser NULL
            mask = (
            ...
            if len(df_match) > 0:
                ...
                no_encontrados.append((sector_ine, tiempo_ine))
                comparisons.append({
                    'Sector': sector_ine[:50],
                    'Métrica': tiempo_ine,
                    'Valor INE': valor_ine,
    ...
    if no_encontrados:
        ...
        for i, (sector, metrica) in enumerate(no_encontrados[:3]):
            print(f"  - {sector[:40]} / {metrica}")
...
def create_validation_excel(df_ine, df_db, df_comparison, output_path):
    ...
    ws2['A1'] = "VALIDACIÓN TABLA 6044 - PERIODO 2025T1"
    ws2['A1'].font = Font(bold=True, size=14)
    ws2['A1'].alignment = Alignment(horizontal='center')
    
    ws2['A3'] = "Sectores B-S (SIN tipo de jornada)"
...
def main():
    """
    Función principal de validación tabla 6044
    """
    print("\n" + "=" * 80)
    print("VALIDACIÓN DETALLADA TABLA 6044")
    print("Nacional + Sectores B-S (SIN tipo de jornada)")
    print("Comparación INE CSV vs DuckDB - Periodo 2025T1")
    print("=" * 80 + "\n")
    
    # Configuración de rutas
    base_dir = Path(__file__).parent
    csv_file = base_dir / "data" / "INE" / "6044.csv"
    db_path = base_dir / "data" / "analysis.db"
    output_file = base_dir / "data" / "INE" / "validacion_6044_detallada.xlsx"
    
    try:
        # 1. Leer y preparar datos del INE
        df_ine, sector_col = read_and_prepare_ine_csv(csv_file)
        
        # 2. Obtener datos de DuckDB
        df_db = get_duckdb_data_detailed(db_path)
        
        # 3. Realizar comparación detallada
        df_comparison = create_detailed_comparison(df_ine, df_db, sector_col)
        ...
        print("\n" + "=" * 80)
        print("VALIDACIÓN COMPLETADA - TABLA 6044")
        print("=" * 80)
        ...
        print(f"\nCaracterística especial tabla 6044:")
        print(f"  - NO tiene tipo de jornada (tipo_jornada = NULL)")
        print(f"  - Solo sectores agregados: Total B-S, Industria, Construcción, Servicios")
